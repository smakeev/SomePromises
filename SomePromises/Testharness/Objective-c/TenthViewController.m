...
{
  __weak TenthViewController *weakSelf = self;
  self.t1btn.enabled = NO;
  self.firstTestLabel.text = @"Label";
  [[[SomePromise promiseWithName:@"10th test promise"
					 resolvers:^(StdBlocks)
					 {
					         int result = 0;
						     for(int i = 0; i < 20; ++i)
							 ...
							 fulfillBlock(@(result));
					 } class: nil] onRejectExecute:^(ElseParams)
					 {
						 dispatch_async(dispatch_get_main_queue(), ^{
							 weakSelf.firstTestLabel.text = @"Finished";
							 fulfillBlock(Void);
						 });
					 } class: nil] onSuccess:^(id result) {
                        dispatch_async(dispatch_get_main_queue(), ^{
